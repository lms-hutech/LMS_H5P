...
H5P.ImageMultipleHotspotQuestion = (function ($, Question) {
  ...
  ImageMultipleHotspotQuestion.prototype.createRetryButton = function () {
    ...
    this.addButton('retry-button', this.params.tryAgain || 'Retry', function () {
      self.resetTask();
    }, false);
  };
   ...
   */
   ImageMultipleHotspotQuestion.prototype.resetTask = function () {
    ...
    // Remove any correct hotspot markers from the DOM
    this.correctHotspotFeedback.forEach(function (feedback) {
      if (feedback.$element) {
