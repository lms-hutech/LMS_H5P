...
const storage = new MongoS3ContentStorage(
    initS3({
        credentials: {
            accessKeyId: 'change-me-access-key', // optional if env. variable is set
            secretAccessKey: 'change-me-secret-key' // optional if env. variable is set
        },
    ...
    (
        await initMongo(
            ...
            'change-me-mongodb-password' // optional if env. variable is set
        )
    ).collection('h5p'),
...
```bash
CONTENTSTORAGE=mongos3 AWS_ACCESS_KEY_ID=change-me-access-key AWS_SECRET_ACCESS_KEY=change-me-secret-key AWS_S3_ENDPOINT="http://127.0.0.1:9000" MONGODB_URL="mongodb://127.0.0.1:27017" MONGODB_DB=testdb1 MONGODB_USER=root MONGODB_PASSWORD=change-me-mongodb-password CONTENT_AWS_S3_BUCKET=testbucket1 CONTENT_MONGO_COLLECTION=h5p npm start
```
